...
void loop()
{
    ...
    millis_time = millis();
    // Check for rollover
    if(millis_time < timeLow ||
       millis_time < timeVeryLow ||
       millis_time < timePress){
        timeLow = millis_time;
        timeVeryLow = millis_time;
        timePress = 0;
    }
        ...
        switch(buttonState) { 
            ...
            case eBUTTON_PRESSED:
                ...
                if(pi_running == true){
                    ...
                    buttonTime = millis_time - timePress;
    ...
    if(pi_running == true){
        ...
        if(millis_time - timeVeryLow > LOW_VOLTAGE_TIME_MS || (
           millis_time - timeLow > LOW_VOLTAGE_TIME_MS &&
           (timePress == 0 || millis_time - timePress > OVERRIDE_TIME_MS))){
            // Start a shutdown
            SleepyPi.piShutdown();
            SleepyPi.enableExtPower(false);
        }
